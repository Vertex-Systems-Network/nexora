...
final class LeadController extends Controller
{
    public function index(Request $request, CurrencyManager $currencies, TenantMemberDirectory $members): Response
    {
        ...
        return Inertia::render('Admin/Crm/Leads',[
            'leads'=>$leads,'filters'=>['status'=>$status],'organizations'=>$this->organizations(),'contacts'=>$this->contacts(),'owners'=>$members->options(),'pipelines'=>$this->pipelines(),
    ...
    public function store(Request $request, CurrencyManager $currencies, CrmTimelineContract $timeline, AutomationEventBusContract $automation): RedirectResponse
    {
        $data=$request->validate([
            ...
            'source'=>['nullable','string','max:120'],'score'=>['required','integer','min:0','max:100'],'currency'=>['nullable','string','size:3'],'estimated_value'=>['nullable','numeric','min:0'],'owner_id'=>['nullable','integer',new TenantMemberExists()],'description'=>['nullable','string','max:5000'],
        ]);
