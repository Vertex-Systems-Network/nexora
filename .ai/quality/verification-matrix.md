...
| `PRIVACY-CONSENT-100` | consent/policy tests | analytics/forms/experiment integration | required | GPC/DNT/export/delete/retention evidence |
...
| `AI-DX-100` | scaffold/review/eval tests | generated package -> SDK/Sentinel/tests | required for representative package | independent review + no unregistered implementation |
| `EXPERIMENTATION-100` | assignment/statistics/goal tests | release/analytics/privacy integration | required | deterministic rollout/rollback/consent evidence |
...
| `DX-200` | CLI/SDK/reference package tests | clean external developer workflow | required for representative environment | docs/compatibility/reference evidence |
...
| `MARKETPLACE-200` | licensing/compat/update tests | publisher/package lifecycle | required | revocation/rollback/transparency evidence |
...
| `MANAGED-CLOUD-100` | provisioning/policy/IaC tests | domains/SSL/CDN/backups/deploy/monitoring | required in managed environment | isolation, metering, restore and scaling evidence |
...
| `OBSERVABILITY-200` | log/metric/trace schema tests | app/jobs/AI/security/ops integration | required | no secret leakage + incident diagnostics |
...
|---|---|
| `PERF-CWV-CERT-100` | production-like performance/CWV/render/query/media/cache evidence |
