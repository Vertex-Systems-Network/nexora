 ...
 *
 * This operator tool coordinates already-authorized, bounded recovery primitives
 * against an explicit target. It never copies source into the target, installs
 * dependencies, runs migrations, or upgrades the target version.
 *
 * Default mode is read-only dry-run. Apply mode requires the exact confirmation
 * token and may only execute a version-specific repair adapter when the observed
 * compatibility mismatch is explicitly supported. After compatibility converges,
 * a stale post-install handoff receipt may be reconciled automatically, followed
 * by an independent readiness re-check and a bounded GET /login HTTP smoke.
 */
...
{
    $text = trim(implode("\n", array_filter($streams, static fn (string $value): bool => $value !== '')));
    if ($text === '') {
        return null;
    }
...
{
    $rows = [];
    foreach ((array) ($compatibility['mismatches'] ?? $compatibility['runtime']['mismatches'] ?? []) as $mismatch) {
...
function nexoraRuntimeRecoveryCompatibilityPass(array $payload): bool
{
...
function nexoraRuntimeRecoveryReady(array $payload): bool
{
    return ($payload['status'] ?? null) === 'pass'
...
function nexoraRuntimeRecoveryLoginSmoke(string $baseUrl): array
{
    ...
    return [
        'status' => $statusCode === null ? 'blocked' : 'fail',
        'url' => $url,
        'http_status' => $statusCode,
        'error' => $statusCode === null
            ? 'Unable to complete verified HTTP(S) /login smoke. Check web-server reachability and PHP CA trust; TLS verification is not disabled.'
            : 'Expected HTTP 200 from /login without following redirects.',
    ];
}
...
function nexoraRuntimeRecoveryWriteReceipt(string $target, array $payload): ?string
{
    ...
    $payload['receipt_sha256'] = hash('sha256', json_encode($copy, JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR));
...
Behavior:
- observes target compatibility first;
- never upgrades/copies source or installs dependencies;
- if rc.93 has only the approved four identity mismatches, delegates to the
  version-pinned rc.93 repair adapter;
- independently re-runs deep compatibility;
- automatically reconciles only a stale post-install receipt when runtime_ready=true;
- re-runs readiness and then performs a verified GET /login smoke;
- writes a machine-readable target receipt for apply-mode outcomes.
TEXT;
}
...
$steps = [];
$compatibilityBefore = nexoraRuntimeRecoveryCompatibility($target);
$compatibilityPayload = $compatibilityBefore['payload'];
...
$steps['compatibility_before'] = [
    'status' => nexoraRuntimeRecoveryCompatibilityPass($compatibilityPayload) ? 'pass' : 'fail',
...
if (! nexoraRuntimeRecoveryCompatibilityPass($compatibilityPayload)) {
    $unsupported = array_values(array_diff($mismatches, NEXORA_RUNTIME_RECOVERY_RC93_ALLOWED_MISMATCHES));
    if (! hash_equals(NEXORA_RUNTIME_RECOVERY_RC93_VERSION, $currentVersion)
        ...
        nexoraRuntimeRecoveryFail('No approved bounded recovery adapter matches the observed target state.', [
            ...
            'installed_version' => $installedVersion,
            'mismatches' => $mismatches,
            'unexpected_mismatches' => $unsupported,
        ]);
    }
    ...
    if (! $apply) {
        nexoraRuntimeRecoveryEmit([
            ...
            'next_action' => 'Re-run with --apply --confirm=RECOVER-RUNTIME to execute repair, verification, stale-receipt reconciliation when required, and /login smoke.',
        ]);
    }

    $compatibilityAfter = nexoraRuntimeRecoveryCompatibility($target);
    $compatibilityPayload = $compatibilityAfter['payload'];
    if (! nexoraRuntimeRecoveryCompatibilityPass($compatibilityPayload)) {
    ...
    $readiness = nexoraRuntimeRecoveryPostInstallStatus($target, false);
    ...
    nexoraRuntimeRecoveryEmit([
        ...
        'steps' => $steps,
        'mutation_performed' => false,
        'next_action' => nexoraRuntimeRecoveryNeedsReceiptRefresh($readiness['payload'])
            ? 'Apply mode can reconcile the stale receipt, re-assert readiness and run /login smoke.'
            : 'Apply mode will re-assert readiness and run /login smoke; no identity repair is planned.',
    ]);
}
...
if (! nexoraRuntimeRecoveryReady($readinessPayload)) {
    ...
    nexoraRuntimeRecoveryFail('Runtime recovery did not reach final post-install readiness.', [
        'target' => $target,
        'readiness' => $readinessPayload,
...
$overallStatus = $loginSmoke['status'] === 'pass' ? 'pass' : 'blocked';
$receipt = nexoraRuntimeRecoveryWriteReceipt($target, [
...
nexoraRuntimeRecoveryEmit([
    ...
    'message' => $overallStatus === 'pass'
        ? 'Runtime recovery closure passed: compatibility, readiness, current handoff receipt and /login HTTP smoke are all PASS.'
        : 'Runtime identity/readiness are PASS, but final /login HTTP smoke could not be certified automatically.',
    'target' => $target,
    'steps' => $steps,
    'evidence_receipt' => $receipt,
    'mutation_performed' => isset($steps['repair_adapter']) || isset($steps['receipt_reconcile']),
    'target_verification_complete' => $overallStatus === 'pass',
], $overallStatus === 'pass' ? 0 : 2);
